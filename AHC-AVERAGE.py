# -*- coding: utf-8 -*-
"""
Created on Tue Jun 27 23:01:09 2023
...
def average_linkage_clustering(points, k):
    ...
    while len(clusters) > k:
        ...
        for i in range(len(clusters) - 1):
            for j in range(i + 1, len(clusters)):
                dist = average_distance(clusters[i].points, clusters[j].points)
...
def silhouette_score(points, clusters):
    ...
    for i, point in enumerate(points):
        ...
        for p in cluster_i.points:
            ...
            sum_distance += distance
            count += 1
        a_i = sum_distance / count if count > 1 else 0
...
result = average_linkage_clustering(points, k)
...
silhouette_scores = []
for k in range(2, 11):
    clusters = average_linkage_clustering(points, k)
